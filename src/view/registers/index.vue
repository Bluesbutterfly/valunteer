<template>
  <div class="goods">
    ...
    <van-cell-group>
      ...
      <textarea name="actContain" placeholder="请输入服务站详情" v-model="data.details" class="van-cell van-hairline van-field"></textarea>
    </van-cell-group>
        ...
        methods: {
            ...
            register(){
                if (this.data.determine == false || this.data.phone == undefined || this.data.pass == undefined){
                    ...
                    this.$axios.post("/api/serviceStation/register",qs.stringify({ "stationPhone":this.data.phone,"password":this.data.pass,"stationName":this.data.stationName,"stationAddress":this.data.address,"presentation":this.data.details })).then(res=>{
                        console.log(res)
