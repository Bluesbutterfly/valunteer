<template>
  <div class="goods">
    ...
    <div class="val-register-href clearfix">
      <router-link  to="/registers/">注册</router-link>
    </div>
  ...
  methods: {
      ...
      login(){
          if (this.data.phone == null || this.data.pass == null){
              ...
              this.$axios.post("/api/serviceStation/login",qs.stringify({ "stationPhone":this.data.phone,"password":this.data.pass })).then(res=>{
                  // console.log(res.data.resData.phone)
                  if (res.data.resCode == -1){
                      this.errorMsg.subErr = "用户名或密码错误"
                  }else {
                      sessionStorage.intergral = res.data.resData.intergral;
                      this.$cookies.set("gid" , res.data.resData.id , 30 * 60)
                      // this.$cookies.set("servePhone",res.data.resData.loginName,30*60)
